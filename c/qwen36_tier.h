 ...
 * q ++ k ++ v fused). All six are offered to the automatic placer, so an
 * unset COLI_PLACE lets it decide; `off` is the way back to experts only, and
 * it is the ONLY granular way back -- naming any one component switches auto
 * off for every component, so a card that wants five of the six has to list
 * all five. attnproj was explicit-only until its A/B ran on 22 September 2026
