...
/* D/I are parameters because the shipping shared expert is 2048x512, not the
 * 64x32 toy: the batch path picks different branches at production width (the
 * OpenMP team threshold, the vector body vs the scalar tail), and a gate that
 * only ever sees the toy shape cannot see those. Removing QWEN36_SHARED_FUSE
 * took the only make-check exactness case that ran the wide shapes with it, so
 * they come back here, against the scalar path instead of against the fused
 * kernel. */
static void shared_case(const char *format,int quantized,int D,int I) {
    enum { S=12 };
    ...
    CHECK(g_qwen_matmul_d_calls==3,"%s decode calls=%llu expected=3",format,
          (unsigned long long)g_qwen_matmul_d_calls);
    printf("qwen shared batch exact: format=%s D=%d I=%d S=%d calls=%d -> 3\n",
           format,D,I,S,S*3);
...
int main(void) {
    ...
    shared_case("f32",0,64,32);
    shared_case("int8",1,64,32);
    shared_case("int8",1,2048,512);  /* the shipping Qwen3.6 shared expert */
    shared_case("int8",1,288,320);   /* scalar tail, shared wider than hidden */
