...
/* Adattato dal test upstream: in questo albero qt_dense_init prende un sesto
 * parametro, la dimensione del gruppo di scale (Q38_TRUNK_GS, che upstream non
 * ha). 0 significa una scala per riga -- esattamente la semantica della
 * versione a cinque parametri da cui questo test proviene, e quella che il
 * ramo gs==0 dell'implementazione esegue. */
int main(void) {
    enum { D=64, I=32 };
    ...
    for(int round=1;round<=2;round++) {
        ...
        CHECK(qt_dense_init(q,sc,D,D,0,0)>=0);
