...
| `COLI_CUDA_I8_ROWS` | `2` | Output rows per block for the fmt=1 **per-row** int8 dense GEMV (`quant_matmul_i8r`); `0` restores the original one-row-per-block kernel, `2`, `4` and `8` are the instantiated widths, anything else reads as the default. The generic branch gives one block to one row and every block reads the **whole** activation vector, so a call moves `I*O` bytes of weights and `I*O*4` of activations — four fifths of the traffic is `x`, and `dense_stats` counts only the weights. At R rows per block `x` is read once per R rows and the total falls from 5× the weight bytes to `(1 + 4/R)×`: **1.5× at R=8, a factor of 3.3**, against the 3.4 that the trunk's measured 29 % of peak leaves on the table. The hot path issues `R*U` independent weight loads (16 in every instantiation) before consuming any, because Nsight Compute measures this kernel as **memory-latency-bound, not bandwidth-bound** — occupancy 91.9 %, DRAM throughput 34.6 %, and 60 % of a 27-cycle issue gap stalled on an L1TEX scoreboard. The summation order per row is unchanged — still `i = t, t+256, …` ascending, still the same reduction tree and trailing f32 scale — so the result is **bitwise identical** and cannot move a logit, which is what `tests/test_int8_rows_cuda.cu` asserts with `memcmp`. **Measured** on an RTX 4070 Ti SUPER (sm_89, qwen36 i4 gs64, clang build, 4×4 alternated + a profiled pass per arm, two sessions; `step()` over 128 generated tokens, kernel GB/s over 64 — compare within a column, not across): the placed dense GEMVs run at **342–381 GB/s against the original's 187–191**, and `step()` falls from **27.25–27.40 to 23.4–23.9 ms/token (−13 %)**. R=2 and R=4 landed inside each other's spread in both sessions (R=4 won one, R=2 the other) and R=8 was the slowest of the three in both, so the default is the cheapest of the two that tied. The `(1 + 4/R)×` traffic model predicts the direction but not the curve — measured/predicted is 1.17–1.22 at R=2 and 0.47–0.51 at R=8 — so the gain is not purely the `x` re-read, and R beyond 4 costs more in registers and occupancy than it saves in traffic. |
| `COLI_CUDA_DOWN_ROWS` | **off** | Output rows per block for the expert **down** projection (`grouped_down_g4r`); `2`, `4` or `8`, anything else off. `grouped_down_g4`'s grid is `(D, rows, chunks)` — one block per output row — and for qwen36 (`D=2048`, `I=512`, top-8) that is **16,384 blocks of 256 threads per launch in which every thread reads exactly one packed byte** (`(I+1)/2 = 256` bytes over 256 threads) before joining a 256-wide reduction with nine `__syncthreads()`. Each block also reads 2048 bytes of activations against 256 bytes of weights, so `x` is **8× the weight traffic** — L2-served, but paid in latency at every one of those blocks. R rows per block is the same change `COLI_CUDA_I8_ROWS` made to the dense GEMV: `x` read once per R rows, the grid down by R, and R independent byte loads per thread instead of one. The per-thread input partition, the multiplication order, the reduction tree and the epilogue are untouched, so the output is **bitwise identical** — `tests/test_grouped_down_rows_cuda.cu` asserts it with `memcmp`. **Off by default, and measured: leave it off.** On an RTX 4070 Ti SUPER (qwen36 i4 gs64, clang, 4×4 alternated) the kernel does what the shape argument said — expert-group kernel time **558 → 438 ms at R=4 (−21.5 %)**, and `qtier take` **0.53 → 0.33 ms/token**, two independent measurements agreeing that the group finishes earlier — and the **token gets 5 % slower** (24.05 → 25.20 ms/token, +1.20 paired, 4/4 repetitions same sign). `moe_total` does not move: `take` was 0.53 ms of a 24 ms token, so the expert kernels were **already off the critical path** and the freed GPU time had nowhere to go. **91 % of the regression is `dn-sub proj` +0.60, `norm+out` +0.20 and `lm_head` +0.39** — the placed dense GEMVs, which run on stream 0 concurrently with the expert group: 4,096 long-lived blocks holding 4 KB of shared memory each leave fewer scheduling slots than 16,384 that retire at once. The kernel stays for the day the expert path lands on the critical path (bigger batch, or `cpu-miss` fixed); today it is not. Full record: `docs/experiments/qwen36-expert-down-rows-2026-09-20-raw.txt`. |
...
| `COLI_CUDA_GRAPH` | **off** | `=1` replays the async expert group as a single `cudaGraphLaunch` instead of five driver calls (descriptor upload, input upload, two kernels, readback). Two measurements point here: `issue` is **1.76 ms/token of CPU spent launching, not waiting**, and the input upload costs **15.6 µs for 8 KiB** — all latency, no bandwidth — so what matters on this path is the *number* of driver calls, which is what a graph removes and what payload reduction (`COLI_CUDA_X_BCAST`) could not. On Windows/WDDM a launch is ~10 µs against 2–3 on Linux, so this is where the gap lives. A graph records **addresses**, so three things are checked rather than assumed: the descriptor staging moved from a stack local to a per-context **pinned** buffer (a captured memcpy from a stack frame replays against memory that is gone), any `reserve()` reallocation bumps a generation counter that throws every captured graph away, and the capture is keyed on `(generation, dispatch arm, count)` so a group whose formats change cannot replay another group's kernels. Only the two packed-int4 arms are captured — the e8 and generic arms can `return` mid-sequence, and a return inside a stream capture leaves the stream captured. `COLI_CUDA_PROFILE=1` takes the ordinary path: four `cudaEventRecord` inside a capture would bake the timing branch into the graph. **Measured** (RTX 4070 Ti SUPER, 2×6 alternated): `issue` **2.11 → 1.51 ms/token**, six pairs of six the same sign — the mechanism works — and **`step()` does not move**, because `take` grows **1.14 → 1.86** and `moe_total` stays at 9.67/9.74. The CPU reaches `qt_take` 0.61 ms earlier and stands still 0.72 ms longer. **The MoE phase is GPU-bound**, so a CPU-side saving converts into waiting; that one fact also explains why `COLI_CUDA_DOWN_ROWS` and `COLI_CUDA_X_BCAST` each measured zero. Off by default, and worth keeping: `take` is now 1.9 ms of headroom a GPU-side saving can fill, which is the 2×2 with `COLI_CUDA_DOWN_ROWS` that the record proposes. Full record: `docs/experiments/qwen36-expert-graph-2026-09-20-raw.txt`. |
| `COLI_CUDA_DENSE_PINNED` | **off** | Pinned host staging for the resident dense GEMV (`coli_cuda_matmul`: `dnproj`, `dnout`, `attnout`, `lm_head`, the shared expert). `1` stages both copies through pinned memory and keeps them synchronous; `2` also makes them async on stream 0 with **one** `cudaStreamSynchronize` instead of two blocking waits; anything else off. **Measured, and it makes the token SLOWER** (RTX 4070 Ti SUPER, qwen36 i4 gs64, clang, 3x6 alternated, identical hit rate and `miss(CPU)` in every arm): `step()` **27.20 -> 28.75 at mode 1 (+1.60 paired, 6/6) and 29.20 at mode 2 (+2.00 paired, 6/6)**. Leave it off. **Why the premise was wrong**, which is the part worth keeping: `h2d` went **140 -> 186 ms** in mode 1, i.e. up 33 % in the arm doing strictly less work. A **pageable** host-to-device `cudaMemcpy` returns once the bytes are in the driver's staging buffer -- the DMA need not have completed -- and every call here is `S=1`, so `xb` is 8 KiB, inside that window. The two "synchronous pageable copies" were not both synchronous: the driver's hidden staging copy was not a cost, it was buying asynchrony for free on a payload where the copy is cheaper than the wait. Mode 2's mechanism did work (`d2h` **171 -> 34 ms**, with the wait moving into the launch/sync residue, 39 -> 160 ms) and the token still lost. Kept as the B arm for a fact worth not re-deriving: on this path the pageable copy is the fast one. The backend prints `[cuda] dense pinned staging active: mode=N` once when the branch fires; a flat A/B without that line means the DLL was never rebuilt. Record: `docs/experiments/qwen36-dense-pinned-2026-09-21-raw.txt`. |
