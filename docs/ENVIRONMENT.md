...
| Variable | Default | Effect |
|---|---|---|
| `COLI_MATMUL_OMP_MIN` | `262144` | FMAs (`S*I*O`) below which `matmul()` (the f32 path in `quant.h`) runs **without** an OpenMP team. An `if` clause on the existing pragma, not a second implementation: the same loop in the same per-output-row order, so the two arms are **bitwise identical by construction** and the change needs no token oracle — `tests/test_matmul_omp_threshold.c` asserts it on six shapes against a double reference. **Why**: qwen36's two DeltaNet gate projections (`in_proj_a`, `in_proj_b`) are `[32 x 2048]` — 65k FMAs, ~4 µs on one core — and they were opening a team each, twice per DeltaNet layer, **60 regions a decode token**; the per-site dense profile put the pair at **0.86 ms/token**, about four times the work they contain, and this project has priced a region at up to 53 µs on a 7950X under MinGW libgomp (`tests/bench_qwen36_decode_omp`). The threshold is conservative — a 2048×2048 matmul is 4.2M FMAs, sixteen times over the line — and `0` restores a team for every shape, which is the B arm. Garbage reads as the default rather than as 0. **Measured only on qwen36's DeltaNet**: every engine's f32 matmuls come through here, and while the change cannot alter a result anywhere it can alter a time, and only one engine's has been looked at. |
