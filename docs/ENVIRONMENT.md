...
| `COLI_CUDA_I8_ROWS` | **off** | Output rows per block for the fmt=1 **per-row** int8 dense GEMV (`quant_matmul_i8r`); `2`, `4` or `8`, anything else off. The generic branch gives one block to one row and every block reads the **whole** activation vector, so a call moves `I*O` bytes of weights and `I*O*4` of activations — four fifths of the traffic is `x`, and `dense_stats` counts only the weights. At R rows per block `x` is read once per R rows and the total falls from 5× the weight bytes to `(1 + 4/R)×`: **1.5× at R=8, a factor of 3.3**, against the 3.4 that the trunk's measured 29 % of peak leaves on the table. Everything else is the original kernel's — the per-thread partition, the byte-at-a-time weight read, the reduction tree, the trailing f32 scale — so the result is **bitwise identical** and cannot move a logit, which is what `tests/test_int8_rows_cuda.cu` asserts with `memcmp`. Off by default because the speed is unmeasured, not because the arithmetic is in doubt. |
