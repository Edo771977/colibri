...
| `COLI_CUDA_PROFILE` | off | Emit CUDA timing, on the synchronous group dispatch and on the async issue/take path decode uses. Off, the async path records nothing and `[qtier] group_stats` says so rather than printing zeros. |
