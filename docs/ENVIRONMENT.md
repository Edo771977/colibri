...
| `QWEN36_SHARED_FUSE` | `0` (off) | Opt-in. At decode (`S=1`) run the CPU shared expert as ONE OpenMP region per layer instead of three: gate row, up row and their SiLU combine in one iteration, then the down GEMV behind a single barrier — the shape `QWEN_EXPERT_KERNEL` already uses for the routed experts. 120 region entries per token become 40 on the 40-layer model. Bit-exact (only the row-to-thread mapping moves; pinned in `tests/test_qwen36_dense_batch.c`), so it is purely a question of whether region entries cost anything on the host. They often do not: where the GEMVs are already at the memory-bandwidth ceiling the saving is inside the noise. Measure with `tests/bench_qwen36_decode_omp` before turning it on. |
