...
| `COLI_CUDA_I8_WARP` | **off** | Opt-in for the fmt=1 **per-row** int8 dense GEMV rework (`quant_matmul_i8w`): `char4`/`float4` loads and a warp shuffle in place of the generic branch's one byte per thread and 256-wide shared-memory tree. That branch is what qwen3.6's placed trunk uses (`lm_head` and `dnproj`, uploaded with `gs=0`), and it measures **194 GB/s against 672 of card** — 29 % of peak, which is the price of a byte-wise GEMV with a block-wide reduction. `=1` selects the rework; any other value, or unset, keeps the original kernel. Grouped int8 (`gs>0`) is untouched: its per-group scale order is part of its CPU-reference contract. **Off by default on purpose** — it changes the summation order (thread `t` took `t, t+256, …`; a lane now takes `4l..4l+3` inside each 128-block), so every logit's last bits move, and this repo gates on token-exact oracles. Verify with `make cuda-test` (`tests/test_int8_warp_cuda.cu`) and the tiny-model oracle before considering a default flip. |
