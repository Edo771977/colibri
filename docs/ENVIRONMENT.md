...
| `CONSIST` | off | `CONSIST=1` runs the engine against itself at two batch sizes: one `step()` call over the whole sequence against a prefix prefill followed by token-by-token decode, over several split points, comparing the logits of the same final position. No oracle and no reference implementation, so it runs on any model, quantization and backend. What it guards is the code that **branches on `S`** — every placement knob is an `S == 1` fast path a batch never takes — plus state continuity across `step()` boundaries (KV length, RoPE positions, the DeltaNet carry). It cannot see a defect the two arms **share**: `consist_reset` gives both the same starting state, so a wrong initialisation or a wrong mask applied to both sides reports zero. That half belongs to the tiny-model oracle. Exits non-zero when the largest relative gap exceeds `CONSIST_TOL`. |
| `CONSIST_TOL` | `1e-2` | Gate for `CONSIST`. The quantity separates the two failure classes: reordered f32 accumulation over the hidden dim lands near `D*eps` (2.4e-4 at hidden 2048), a wrong mask or a misaddressed KV row lands at O(1) — measured, a skipped decode-only state update gives 0.25–0.30 and an `S == 1` RoPE off-by-one 0.04–0.06. An error confined to one layer can still dilute through the residual stream and the lm_head before it is measured. Argmax flips are reported but never gated — a flip requires the top two within `2*gap` by construction. |
