...
| Variable | Default | Effect |
|---|---|---|
| `COLI_MATMUL_OMP_MIN` | **0** (off) | FMAs (`S*I*O`) below which `matmul()` (the f32 path in `quant.h`) runs **without** an OpenMP team. `0` means a team for every shape — the behaviour before this knob existed — and that is the default, because **it was measured and it is worth nothing here**. An `if` clause on the existing pragma, not a second implementation: same loop, same per-output-row order, so the arms are **bitwise identical by construction** and no oracle is needed; `tests/test_matmul_omp_threshold.c` asserts it on six shapes against a double reference. **The measurement** (RTX 4070 Ti SUPER host, clang/LLVM libomp, `OMP_WAIT_POLICY=ACTIVE`, 2×6 alternated, identical generation): `dn-sub proj` **+0.05** `[-0.20 0.10 -0.20 0.10 0.00 0.10]`, `step()` −0.15 with signs disagreeing, and `attention` — the control — flat at 5.94 vs 5.86. **Why**, and it is arithmetic: the 53 µs region this was aimed at is MinGW libgomp's; under libomp with the threads already spinning a region costs single-digit µs, so a team is ~3.3 µs against ~4.3 µs serial for the `[32 x 2048]` DeltaNet gate shape — **exactly break-even**. A threshold calibrated for a 53 µs region is also far too high for a 3 µs one and would switch the team off where it still pays, so the right value belongs to an OpenMP runtime, not to a shape. **Worth trying** where a region really is that expensive: this project measured 57.70 ms/token under MinGW libgomp against 32.73 under libomp on the same commit, and there 60 regions a decode token is ~3 ms. Never measured outside qwen36's DeltaNet. Record: `docs/experiments/qwen36-matmul-omp-min-2026-09-21-raw.txt`. |
