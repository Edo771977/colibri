...
| `COLI_CUDA_DENSE_PINNED` | **off** | Pinned host staging for the resident dense GEMV (`coli_cuda_matmul`: `dnproj`, `dnout`, `attnout`, `lm_head`, the shared expert). `1` stages both copies through pinned memory and keeps them synchronous; `2` also makes them async on stream 0 with **one** `cudaStreamSynchronize` instead of two blocking waits; anything else off. Three values because there are two mechanisms here — the driver's hidden staging out of pageable memory, and the two blocking round-trips — and one flag changing both would give a number nobody could attribute. **The measurement that motivates it** (`COLI_CUDA_PROFILE=1`, RTX 4070 Ti SUPER, qwen36 i4 gs64, clang, R=2 arm, 5224 calls over 64 tokens): kernel 301 ms, h2d 96 ms, d2h 120 ms, launch+sync residue 71 ms, **wall 588 ms** — i.e. **81.6 calls a token, 57.6 µs of kernel and 54.9 µs of everything else per call, 4.48 ms/token outside the kernels.** Unlike the expert group, these copies are **synchronous**: the engine thread is stopped inside them and no CPU window is covering them, which is why this is a different bet from `COLI_CUDA_DOWN_ROWS` and `COLI_CUDA_X_BCAST`. Staging changes where the bytes are read from, never their value, so the contract is **bitwise identity** across all three modes; `tests/test_dense_pinned_cuda.cu` asserts it with `memcmp` and interleaves mode 2 with the pageable path on changing inputs, because a missing synchronize returns the *previous* call's answer rather than failing. **Off by default and not yet measured on a token** — the profile above is real, whether it moves `step()` is not yet known, and three correct optimisations in this area have already been worth zero. The backend prints `[cuda] dense pinned staging active: mode=N` once when the branch fires; a flat A/B without that line means the DLL was never rebuilt. |
